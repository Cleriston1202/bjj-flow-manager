  ...
  async function performCheckin(studentId: string, source: 'scan' | 'manual' = 'manual') {
    ...
    try {
      ...
      let { data: payments, error: payError } = await supabase
        ...
        .order('start_date', { ascending: false })  // Buscar pagamentos mais recentes primeiro
      ...
      if (!payments || payments.length === 0) {
        ...
        console.log('[CHECK-IN] Nenhum pagamento encontrado. Criando automaticamente...')
        ...
        if (createErr) {
          console.error('[CHECK-IN] Erro ao criar pagamento:', createErr.message)
        } else {
          console.log('[CHECK-IN] Pagamento criado com sucesso')
          ...
          console.log('[CHECK-IN] Pagamentos após criar:', payments)
        }
      }
      ...
      console.log('[CHECK-IN] Status final computado:', status)
      console.log('[CHECK-IN] Dados do pagamento:', payment ? {
        id: payment.id,
      ...
      // Bloqueio APENAS se delinquent (NÃO bloquear por 'late' ou 'pending')
      if (status === 'delinquent') {
        console.log('[CHECK-IN] ❌ BLOQUEANDO - Status delinquent')
        setMessage('Acesso bloqueado. Favor passar na recepção.')
        setMessageType('error')
        return
      }

      console.log('[CHECK-IN] ✅ Prosseguindo com check-in')
